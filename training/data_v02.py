...
from amber.dataset_v02 import (
    TRAIN_FILE as V02_TRAIN_FILE,
    VALIDATION_FILE as V02_VALIDATION_FILE,
)
from tokenizer.amber_bpe import AmberBPETokenizer


ROOT = Path(__file__).resolve().parents[1]
CACHE_DIR = ROOT / "data" / "v02" / "cache"
...
def _encode_file(
    ...
    if not source.exists():
        raise FileNotFoundError(
            f"Dataset Amber 0.2 introuvable : {source}"
        )
    ...
    return {
        ...
        "cache_size_bytes": destination.stat().st_size,
...
def build_v02_cache():
    ...
    print(
        "AMBER 0.2 TOKEN CACHE BUILDER",
        flush=True,
    )

    print(
        (
            "Tokenizer conservé depuis Amber 0.1 : "
    ...
    print(
        (
            "[V02 CACHE COMPLETE] "
            f"train={train['tokens']:,} | "
            f"validation={validation['tokens']:,}"
        ),
        flush=True,
    )
